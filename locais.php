...
if (!$resultado1) {
  die("Erro na consulta submenu_contactos: " . mysqli_error($conn));
}
?>
...
<head>
  ...
  <title>Locais a Visitar - Junta de Freguesia de Barreiro de Besteiros</title>
  ...
  <style>
    ...
    /* Breadcrumbs iguais aos da outra página */
    .breadcrumbs {
      position: relative;
      bottom: 70px;
      text-align: center;
...
<body>
  ...
  <div class="page-wrapper fade-out">
    ...
    <section class="img_inicial">
      <div class="texto">
        ...
        if ($result && $result->num_rows > 0):
          while ($row = $result->fetch_assoc()): ?>
            <h1 class="tituloprincipal"><?= htmlspecialchars($row['titulo']) ?></h1>
            <h2 class="descricao"><?= nl2br(htmlspecialchars($row['subtitulo'])) ?></h2>
          <?php endwhile;
        else: ?>
          <h1 class="tituloprincipal">Locais de Interesse</h1>
          <h2 class="descricao">Descubra os principais pontos da nossa freguesia.</h2>
        <?php endif; ?>
      </div>
    </section>

    <main class="breadcrumbs">
      <a href="index.php">Início</a> <i class="fa-solid fa-angle-right"></i> <a href="locais.php"> Locais a Visitar</a>
    </main>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const setaEsq = document.getElementById('seta-esquerda-locais');
      const setaDir = document.getElementById('seta-direita-locais');
  ...
  <!-- Script para o efeito de fade-in -->
  <script>
