...
$sql_executivo = "SELECT titulo_pagina, descricao, img_banner FROM executivo LIMIT 1";
$resultado3 = mysqli_query($conn, $sql_executivo);
...
if (!$resultado3) {
  die("Erro na consulta executivo: " . mysqli_error($conn));
}
?>
...
<head>
  ...
  <title>Executivo - Junta de Freguesia de Barreiro de Besteiros</title>
  ...
  <style>
    .img_inicial {
      ...
      background: url('IMAGENS/20250706_174232.jpg') center center/cover no-repeat;
      height: 430px;
      display: flex;
      align-items: center;
...
<div class="jf-search-bar">
  ...
  <section class="img_inicial">
    <div class="texto">
      <?php if ($resultado3 && $resultado3->num_rows > 0): ?>
        <?php while ($row = $resultado3->fetch_assoc()): ?>
          <h1 class="titulo"><?= htmlspecialchars($row['titulo_pagina']) ?></h1>
  ...
  <main class="breadcrumbs">
    <a href="index.php">Ínicio</a> <i class="fa-solid fa-angle-right"></i> Executivo
  </main>
  ...
  <div class="page-wrapper">
    <section class="executivo-grid">
      <?php
      $sql = "SELECT nome, cargo, img, email, telefone FROM executivo";
      $result_executivo = mysqli_query($conn, $sql);

      if ($result_executivo && mysqli_num_rows($result_executivo) > 0):
        while ($executivo = mysqli_fetch_assoc($result_executivo)): ?>
          <article class="member-card">
            <div class="member-photo-wrapper">
              <img src="IMAGENS/<?php echo htmlspecialchars($executivo['img']); ?>"
                alt="<?php echo htmlspecialchars($executivo['nome']); ?>">
            </div>
            <div class="member-body">
              <div class="member-name">
                <?php echo htmlspecialchars($executivo['nome']); ?>
              </div>
              <div class="member-role">
                <?php echo htmlspecialchars($executivo['cargo']); ?>
              </div>
              <div class="member-actions">
                <a href="mailto:<?php echo htmlspecialchars($executivo['email']); ?>"
                  title="Enviar e‑mail">
                  <i class="fa-solid fa-envelope"></i>
                </a>
                <a href="tel:+351<?php echo htmlspecialchars($executivo['telefone']); ?>"
                  title="Telefone">
