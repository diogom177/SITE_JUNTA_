...
$sqlAvisos = "SELECT id_aviso, titulo_aviso, corpoaviso, data FROM avisos";
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sqlAviso = "SELECT id_aviso, titulo_aviso, corpoaviso, data, img FROM avisos WHERE id_aviso = " . $id_aviso . " LIMIT 1";
...
<head>
    ...
    <title><?= htmlspecialchars($aviso['titulo_aviso']) ?></title>
    ...
    <style>
        ...
        .content {
            ...
            text-align: center;
            overflow: hidden;
            background: url('IMAGENS/<?= htmlspecialchars($aviso['img']) ?>') center / cover no-repeat;
...
<body>
    <?php
    $sqlavisos1 = "SELECT titulo_pagina, descricao FROM avisos LIMIT 1";
    $resultavisos1 = $conn->query($sqlavisos1);
    ?>
    ...
    <div class="page-wrapper fade-out">
        ...
        <main class="main-content">
            <div class="content">
                <div class="texto">
                    <h2><?= htmlspecialchars($aviso['titulo_aviso']) ?></h2>
                </div>
            </div>
        </main>

        <main class="breadcrumbs">
            <a href="index.php">Ínicio</a> <i class="fa-solid fa-angle-right"></i> <a href="avisos.php"> Avisos</a> <i class="fa-solid fa-angle-right"></i>
            <?= htmlspecialchars($aviso['titulo_aviso']) ?></a>
        </main>


        <main style="bottom: 150px; position:relative; right:160px; align-items:center;">
            <img class="noticia-imagem" src="IMAGENS/<?= htmlspecialchars($aviso['img']) ?>" alt="<?= htmlspecialchars($aviso['titulo_aviso']) ?>">
            <div class="noticia-data"><?= formatarDataPt($aviso['data']) ?></div>
            <i class="bi bi-calendar" style="width: 30px; height:30px; margin-left: 314px; position:relative; font-size:19px;"></i>
            <div class="noticia-titulo" style="position:relative; left: 110px;"><?= htmlspecialchars($aviso['titulo_aviso']) ?></div>
            <div class="noticia-lead"><?= ($aviso['corpoaviso']) ?> </div>
