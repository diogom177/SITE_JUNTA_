  ...
  <div class="page">
    ...
    <nav class="paginacao">
      <ul>
        ...
        // Botão anterior
        if ($paginaAtual > 1) {
          echo '<li><a href="?pagina=' . ($paginaAtual - 1) . '"><</a></li>';
        } else {
          echo '<li><span class="disabled"><</span></li>';
        }

        // Cálculo do range (max 7 páginas)
        $max = 7;
        $meio = floor($max / 2);
        $inicio = max(1, $paginaAtual - $meio);
        $fim = min($totalPaginas, $inicio + $max - 1);
        if ($fim - $inicio + 1 < $max) {
          $inicio = max(1, $fim - $max + 1);
        }
        ...
        // Botão última página
        if ($paginaAtual < $totalPaginas) {
          echo '<li><a href="?pagina=' . $totalPaginas . '">»</a></li>';
        } else {
          echo '<li><span class="disabled">»</span></li>';
        }
        ?>
