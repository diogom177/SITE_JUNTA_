...
$sqlHeader = "SELECT titulo, descricao, img_banner FROM atas LIMIT 1";
...
<head>
    ...
    <title>Atas - Junta de Freguesia de Barreiro de Besteiros</title>
...
<body>
    ...
    <section class="img_inicial">
        <div class="texto">
            ...
            ?>
                <h1 class="tituloprincipal"><?= htmlspecialchars($row_header['titulo']) ?></h1>
    ...
    <main class="breadcrumbs">
        <a href="index.php">Ínicio</a> <i class="fa-solid fa-angle-right"></i> <a href="atas.php"> Atas</a>
    </main>
    ...
    <!--TROCAR ISTO??????????????????????????????????????????????????????????????????????????????????????????????-->
    <script>
        // Script para alternar a exibição do conteúdo do ano
        document.querySelectorAll('.ano-toggle').forEach(button => {
            button.addEventListener('click', () => {
                const anoAtas = button.parentElement;
                anoAtas.classList.toggle('open');
            });
        });
    </script>
</body>
