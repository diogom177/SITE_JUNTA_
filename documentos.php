...
$sqlHeader = "SELECT titulo_banner, descricao2 FROM documentos";
$resultHeader = $conn->query($sqlHeader);

if (!$res1 || !$resultado || !$resultado1 || !$result2 || !$resultHeader) {
...
<body>
    ...
    <div class="texto">
      <?php if ($resultHeader && $resultHeader->num_rows > 0): ?>
        <?php while ($row = $resultHeader->fetch_assoc()): ?>
          <h1 class="tituloprincipal"><?= htmlspecialchars($row['titulo_banner']) ?></h1>
  ...
  <main class="breadcrumbs">
    <a href="index.php">Ínicio</a> <i class="fa-solid fa-angle-right"></i> <a href="documentos.php">Pedido de Documentos</a></a>
  </main>
  ...
  <style>
    ...
    .img_inicial {
      ...
      background: url('IMAGENS/junta.jpg') center center/cover no-repeat;
      height: 430px;
      display: flex;
      align-items: center;
  ...
  <section class="documentos-section">
    <div class="contact-container">
      <h2>Pedido de Documentos</h2>
      <p class="subtitle">Preencha o formulário para solicitar documentos oficiais</p>

      <form id="formDocumentos" method="post" action="enviar_documentos.php">
        <!-- Nome + Email -->
        <div class="row-2">
          <div class="field">
        ...
        <div class="field">
          <select id="tipo_doc" name="tipo_doc" required>
            ...
            <option value="declaracao_obras">Declaração de Obras</option>
            <option value="outro">Outro</option>
          </select>
          <label for="tipo_doc" class="label">Tipo de Documento</label>
          <div class="underline"></div>
        </div>
        ...
        <div class="row-2">
          <div class="field">
            ...
            <label for="morada" class="label">Morada Completa</label>
            <div class="underline"></div>
          </div>
          <div class="field">
        ...
        <div class="field field-mensagem">
          <textarea id="detalhes" name="detalhes" rows="5" placeholder=""></textarea>
          <label for="detalhes" class="label-mensagem-cinzenta">Detalhes / Data Necessária</label>
        </div>

        <button class="send-btn" type="submit">
          Solicitar Documento <i class="bi bi-file-earmark-arrow-up"></i>
        </button>
      </form>
    </div>
  </section>
