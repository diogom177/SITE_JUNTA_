...
$sqlHeaderQuery = "SELECT titulo, descricao FROM sobre_freguesia LIMIT 1";
$resultHeaderQuery = $conn->query($sqlHeaderQuery);

if (!$res1 || !$resultado || !$resultado1 || !$result2) {
...
<head>
  ...
  <title>Sobre a Freguesia - Junta de Freguesia de Barreiro de Besteiros</title>
...
<body>
    ...
    <div class="texto">
      <?php if ($resultHeaderQuery && $resultHeaderQuery->num_rows > 0): ?>
        <?php while ($headerRow = $resultHeaderQuery->fetch_assoc()): ?>
          <h1 class="tituloprincipal"><?= htmlspecialchars($headerRow['titulo']) ?></h1>
  ...
  <main class="breadcrumbs">
    <a href="index.php">Ínicio</a> <i class="fa-solid fa-angle-right"></i> <a href="sobre.php"> Sobre a Freguesia</a></a>
  </main>
